...
function render() {
    ...
    // 2. Group by Rayon
    const grouped = {};
    // Ensure all known rayons exist even if empty? No, better show only what's there.
    // Actually user wants to see populated rayons.

    // Initialize groups based on constant list to keep order? 
    // Or just dynamic? Let's use dynamic but sort keys by RAYONS order.
    ...
    sortedRayons.forEach(rayon => {
        ...
        // Open only if searching or viewing "In List"
        details.open = (currentFilter.length > 0 || filterMode === 'inList');
        ...
        groupArticles.forEach(article => {
            ...
            const statusBadge = isInList
                ? `<span class="status-badge status-active"><span class="material-symbols-rounded" style="font-size:16px;">shopping_cart</span> Dans la liste</span>`
                : `<span class="status-badge status-inactive">En rayon</span>`;
            ...
            tr.innerHTML = `
                ...
                <td><code style="background:#f0f0f0; padding:2px 4px; border-radius:4px; font-size: 0.9em">${escapeHtml(article.reference || "-")}</code></td>
                <td>${statusBadge}</td>
