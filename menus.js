...
function createMenuCard(menu) {
    ...
    card.innerHTML = `
        ...
        <div class="menu-card-actions">
            ...
            <button class="btn-icon btn-recipe" title="Voir/Éditer la recette" onclick="window.openRecipeModal('${menu.id}')">
                <span class="material-symbols-rounded">menu_book</span>
            </button>
            ${!isInWeekMenu ? `<button class="btn-icon" title="Modifier" onclick="window.editMenu('${menu.id}')">
                <span class="material-symbols-rounded">edit</span>
            </button>` : ''}
            ${!isInWeekMenu ? `<button class="btn-icon" style="color:var(--danger)" title="Supprimer" onclick="window.deleteMenu('${menu.id}')">
...
// --- Recipe Modal ---
let currentRecipeMenuId = null;

window.openRecipeModal = (id) => {
    const menu = menus.find(m => m.id === id);
    if (!menu) return;

    currentRecipeMenuId = id;
    document.getElementById("recipeMenuId").value = id;
    document.getElementById("recipeModalTitle").textContent = `📖 Recette: ${menu.title}`;
    document.getElementById("recipeContent").value = menu.recipe || "";
    document.getElementById("recipeModal").classList.remove("hidden");
};

window.closeRecipeModal = () => {
    document.getElementById("recipeModal").classList.add("hidden");
    currentRecipeMenuId = null;
};

window.insertEmoji = (emoji) => {
    const textarea = document.getElementById("recipeContent");
...
window.saveRecipe = async () => {
    if (!currentRecipeMenuId) return;

    const recipeContent = document.getElementById("recipeContent").value;

    try {
        await updateDoc(doc(db, "menus", currentRecipeMenuId), {
